class CreatedStory < ApplicationRecord
    
   
    
    has_many :user_created_stories
    has_many :users, through: :user_created_stories

    validates :main_character, presence: true
    validates :options_type, presence: true
    validates :settings_type, presence: true
    validates :live_type, presence: true
    ...
    def create_story
        ...
        if options_type == "#/slasher"
            ...
            if settings_type == "#/mythology" && live_type == "#/live" 
                ...
                explanation = "There on the mantle of the frat house’s fireplace was the fearsome zombie’s great-grandfather’s fraternity pin. The old man had told #{main} that you needed something linked to the monster’s bloodline in order to finally slay it"
                tool = "fraternity pin"
                live = "The massive zombie stumbled towards #{main}. #{main} could feel its putrid breath seep out from under the hockey mask. #{main} new it was now or never—they lunged forward and jabbed the pin into the beast’s chest. Tragically, a pin doesn’t go that deep… even if enchanted, it isn’t like it is going to really harm this hulk of a zombie killer. The killer laughed as it slammed that massive machete through #{main} over and over again… #{main}’s cries echoing throughout this town of the damned."
            ...
            x = "#{main} gasped as #{place} fell to pieces around them. The horrible #{monster} was destroying everything in #{city}. #{monster} towered over everything in sight. Why was this happening to #{main}’s home? The #{monster}’s giant foot smashed the restaurant where #{main} had their first kiss. #{main} was furious, but they thought back to how this all started.
